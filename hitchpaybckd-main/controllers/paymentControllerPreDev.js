...
const { formatAmount, ucFirst, cleanMe, shAcessToken, psb9Token, getFee, TransLimit, FreeTransfersCount, updateBalance, calculateProfitAndFee,
    logReferEarn, dailyBonus, toTwoDecimal, applyTaskBonus, applyCouponDiscount
        ...
        if (userbal > 0 && userbal >= topay) {
            ...
            try {
                ...
                if (externalApiResponse.responseCode == '00') {
                    ...
                    await Payn.update(
                        { status: 1, productid: details.transactionId, jsonresp: jsonString, meta: JSON.stringify(meta_data_final), revenue: calculatedProfit, providerfee: actualProviderFee },
...
const buyAirtime = async (ntwk, recipientno, amount, txref) => {
    ...
    const billProvider = 'vtpass'; //testing

    if (billProvider == "coralpay") {
        ...
        let data = {
            // "request_id": "202601071830YUs83iroeikd",
            "request_id": txref,
            "serviceID": ntwk.toLowerCase(),
            "amount": amount,
            "phone": recipientno
        };
        ...
        let timed = Date.parse(new Date()) / 1000;
        // try {
        //     await LogRequest.create({ reference: txref, jsonreq: data, timed: timed, product: 'airtime', provider: 'vtpass' });
        // } catch (logError) {
        //     console.error("Failed to log request for buyAirtime:", logError.message);
        // }

        /* ======================logreuest end ============================== */
        // call the vtpass airtime purchase api
        var config = {
            method: 'POST',
            url: `${process.env.VTPASS_URL}/pay`,
            headers: {
                'Content-Type': 'application/json',
                "api-key": process.env.VTPASS_APIKEY,
                "secret-key": process.env.VTPASS_SKEY
            },
            data: data
        };
        ...
        try {
            ...
            console.log('vtpass airtime response', theData)

            if (theData.code === '000') {
                return {
                    responseCode: theData.code,
                    message: theData.response_description,
                    status: 'success',
                    responseData: {
                        status: theData.content.transactions.status,
                        product_name: theData.content.transactions.product_name,
                        unique_element: theData.content.transactions.unique_element,
                        unit_price: theData.content.transactions.unit_price,
                        quantity: theData.content.transactions.quantity,
                        service_verification: theData.content.transactions.service_verification,
                        channel: theData.content.transactions.channel,
                        commission: theData.content.transactions.commission,
                        total_amount: theData.content.transactions.total_amount,
                        discount: theData.content.transactions.discount,
                        type: theData.content.transactions.type,
                        email: theData.content.transactions.email,
                        phone: theData.content.transactions.phone,
                        name: theData.content.transactions.name,
                        convinience_fee: theData.content.transactions.convinience_fee,
                        amount: theData.content.transactions.amount,
                        platform: theData.content.transactions.platform,
                        method: theData.content.transactions.method,
                        transactionId: theData.content.transactions.transactionId,
                        commission_details: theData.content.transactions.commission_details
                    }
                };
                ...
                return {
                    ...
                    message: theData.response_description,
                    status: 'failed',
                    responseData: theData.content ? theData.content.transactions : null
                };
            }

        } catch (error) {

            logger.error(`VtPass API Error for buyAirtime (txref: ${txref}): ${error}`);
            return {
                status: 'error', // or 'failed'
                responseCode: error.response ? error.response.status : 'NETWORK_ERROR', // Or a custom code
                message: error.response ? (error.response.data.response_description || error.message) : 'Network error or timeout with provider',
                responseData: null
            };
        }
...
const buyData = async (recipientno, amount, txref, productid, network) => {
    const getsett = await AppSett.findOne({ where: { id: 1 } });
    // const billProvider = getsett.billprovider;
    const billProvider = 'vtpass'; //testing
    ...
    if (billProvider == 'vtpass') {
        ...
        let timed = Date.parse(new Date()) / 1000;
        // try {
        //     await LogRequest.create({ reference: txref, jsonreq: data, timed: timed, product: 'databundle', provider: 'VtPass' });
        // } catch (logError) {
        ...
        try {
            ...
            logger.error(`VtPass API Error for buyData (txref: ${txref}):`, { error: error.message, response: error.response?.data });
            return {
                status: 'error',
...
const buyCable = async (recipientno, amount, txref, productid, custname, ntwk, custphone, subscription_type = 'renew') => {
    const getsett = await AppSett.findOne({ where: { id: 1 } });
    // const billProvider = getsett.billprovider;
    const billProvider = 'vtpass'; //testing

    var timed = Date.parse(new Date()) / 1000;
    if (billProvider == "coralpay") {
        let data = JSON.stringify({
        ...
        let data = {
            "request_id": txref,
            "serviceID": ntwk.toLowerCase(),
            "billersCode": recipientno, //smart card number
            "phone": custphone,
            "amount": amount,
            "variation_code": productid,
        ...
        try {
            await LogRequest.create({ reference: txref, jsonreq: data, timed: timed, product: 'cabletv', provider: 'vtpass' });
        } catch (logError) {
            console.error("Failed to log request for buyCable:", logError.message);
        }
        ...
        try {
            let response = await axios.request(config);
            const theData = response.data;
            console.log('vtpass cable response', theData)
            ...
            if (theData.code === '000') {
                return {
                    ...
                    responseData: {
                        ...
                        phone: theData.content.transactions.phone,
                        name: theData.content.transactions.name,
                        convenience_fee: theData.content.transactions.convinience_fee,
                        amount: theData.content.transactions.amount,
                        platform: theData.content.transactions.platform,
                        method: theData.content.transactions.method,
                        transactionId: theData.content.transactions.transactionId
            ...
            logger.error(`VtPass API Error for buyCable (txref: ${txref}):`, { error: error.message, response: error.response?.data });
            return {
                status: 'error',
                responseCode: error.response ? error.response.status : 'NETWORK_ERROR',
                message: error.response ? (error.response.data.message || error.message) : 'Network error or timeout with provider',
...
const vendElect = async (recipientno, amount, txref, vendtype, custname, userphoneno, variation_code) => {
    ...
    if (billProvider == "coralpay") {
        ...
        try {
            let response = await axios.request(config);
            const theData = response.data;
            console.log('coralpay electricity response', theData)
            if (theData.code !== '000') {
                return {
                    responseCode: theData.code,
                    message: theData.response_description,
                ...
                return {
                    ...
                    responseData: {
                        ...
                        discount: theData.content.transactions.discount,
                        type: theData.content.transactions.type,
                        purchased_code: theData.purchased_code,
                        CustomerName: theData.CustomerName,
                        CustomerAddress: theData.CustomerAddress,
                        ReceiptNumber: theData.ReceiptNumber,
                        Amount: theData.Amount,
                        Tax: theData.Tax,
                        Units: theData.Units,
                        Token: theData.Token,
                        Tariff: theData.Tariff,
            ...
            logger.error(`CoralPay API Error for vendElect (txref: ${txref}): ${error}`);
            return {
                status: 'error',
                responseCode: error.response ? error.response.status : 'NETWORK_ERROR',
                message: error.response ? (error.response.data.message || error.response.data.response_description || error.message) : 'Network error or timeout with provider',
                responseData: null
            };
        }
        ...
        try {
            ...
            await LogRequest.create({ reference: txref, jsonreq: payload, timed: timed, product: 'electricity', provider: 'vtpass' });
        } catch (logError) {
            console.error("Failed to log request for vendElect:", logError.message);
        }
        /* ======================logreuest end ============================== */
        ...
        try {
            ...
            logger.error(`VtPass API Error for vendElect (txref: ${txref}):`, { error: error.message, response: error.response?.data });
            return {
                status: 'error',
                responseCode: error.response ? error.response.status : 'NETWORK_ERROR',
                message: error.response ? (error.response.data.message || error.message) : 'Network error or timeout with provider',
...
vendElect('1111111111111', '2000', 'HTCH34596743403', 'ibadan', 'John Doe', '08012345678', 'prepaid')
...
const transferPayment = async (req, res) => {
    const userid = req.user.id;
    let { amount, recipientno, bankname, bankcode, accountname, isbeneficiary, narration, enquirytoken, transpin, envroute, currency, accounttype } = cleanMe(req.body);
    ...
    // if (!currency || (currency == '')) return res.status(400).json({ status: false, message: 'Payment currency is required' });
    ...
    try {
        ...
        if (userbal < topay)
            return res.status(400).json({ status: false, message: `Insufficient balance for ${currency}${formatAmount(topay)}.` });
        ...
        if (bankcode.toLowerCase() === 'hitchpay') {
            ...
            // const selfAccountCheck = await Bank.findOne({ where: { userid: userid, accountno: recipientno} });
            // if (selfAccountCheck) return res.status(400).json({ status: false, message: 'You cannot transfer to your own linked bank account.' });
        }
            ...
            try {
                ...
                if (FTProvider.toLowerCase() == 'safehaven') {
                    ...
                    const payload = JSON.stringify({
                        ...
                        narration: `${fname} ${lname} - ${pay_desc_transfer}`, paymentReference: txref
                    });
                    ...
                    const payload = JSON.stringify({
                        ...
                        order: {
                            ...
                            description: `${fname} ${lname} - ${pay_desc_transfer}`,
                            currency: "NGN",
                            country: "NGA"
                        },
                        customer: {
...
const initiatePay = async (req, res) => {
    try {
        ...
        if (prdid) {
            ...
            checkFee = await Product.findOne({ where: { category: theProduct, status: 1 } });
        }

        if (!checkFee) return res.status(400).json({ status: false, message: 'Selected product is currently not available.' });

        const prdamnt = checkFee?.status == 1 ? parseFloat(checkFee.amount) : 0;
        const providerAmount = parseFloat(checkFee.providerprice) || 0;
        let topay;
        let amountval_service = parseFloat(amount); // Default to amount for most services
        let dfee_hitchpay;

        if (theProduct == 'airtime') {
            topay = parseFloat(amount); // No discount logic shown for initiatePay, assuming direct amount
            amountval_service = parseFloat(amount);
            dfee_hitchpay = 0;
        } else if (theProduct == 'databundle') {
            topay = prdamnt; // Selling price
            amountval_service = providerAmount; // Cost price from provider
            dfee_hitchpay = topay - providerAmount;
            productCodeForExternalCall = checkFee.prdcode;
        } else {
            topay = parseFloat(amount) + prdamnt;
            amountval_service = parseFloat(amount);
            dfee_hitchpay = prdamnt;
        }

        const txref = 'HTCH' + md5(randomstring.generate(5)).toUpperCase().substring(0, 10);
        let timed = Date.parse(new Date()) / 1000;

        // ... (validations for amount limits, phone length, etc. as in processPayment)
        ...
        await OfflinePay.create({
            ...
            recipient: recipientno, jsonreqst: logreq_json
        });
        ...
        <div>
            <div class="greybg" style="padding: 30px 20px;">
                <p style="font-style: normal; font-weight: 400; font-size: 20px; line-height: 36px; letter-spacing: 0.025em; color: #101010; text-align: left;">
                    ...
                    You initiated an online ${product} payment. Kindly proceed with your payment.<br>
        ...
        mailSender('', 'Online Payment - HitchPay', email, emailContent);
...
async function logDailyRevenue(req, res) {
    ...
    try {
        ...
        const txref = 'HCH' + md5(randomstring.generate(3) + 'RV').toUpperCase().substring(0, 10);
...
const withdrawEarning = async (req, res) => {
    try {
        ...
        const txref = 'STP' + md5(randomstring.generate(3) + 'EARN' + userid).toUpperCase().substring(0, 12);
        let timed = Date.parse(new Date()) / 1000;
...
const collateDailyCheckoutSettlements = async (req, res, next) => {
    ...
    try {
        ...
        await t.rollback();
        logger.error('Error in collateDailyCheckoutSettlements:', error);
        next(error);
    }
};

module.exports = {
    processPayment, transferPayment, BeneficiaryList, initiatePay,
